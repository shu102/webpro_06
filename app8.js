...
app.post("/like", (req, res) => {
    ...
    console.log("いいね対象 ID:", id);
    if (id >= 0 && id < bbs.length) {
      bbs[id].likes = (bbs[id].likes || 0) + 1;  // 初期化してからインクリメント
  ...
  app.post("/post", (req, res) => {
    ...
    const message = req.body.message;
    console.log([name, message]);
    bbs.push({ name: name, message: message, likes: 0 });  // likes プロパティを初期化
    res.json({ number: bbs.length });
});
...
app.put("/edit/:id", (req, res) => {
    ...
    console.log("編集対象 ID:", id, "新しいメッセージ:", newMessage);
    if (id >= 0 && id < bbs.length) {
      bbs[id].message = newMessage;  // メッセージを更新
      res.json({ success: true, updatedMessage: bbs[id] });
    } else {
      res.status(400).json({ error: "Invalid ID" });
    }
  });
